...
export const EVENT_PLATFORM_CHART_COLORS: Record<string, string> = {
  Web: "#60a5fa",
  iOS: "#c084fc",
  Android: "#34d399",
  Other: "#94a3b8",
...
export function EventsPlatformDonut({
  slices,
  rangeLabel,
}: {
  slices: EventsPlatformSlice[];
  rangeLabel: string;
}) {
  ...
  return (
    <AnalyticsPanel aria-label="Events by platform">
      <AnalyticsPanelHeader
        ...
        <div className="grid gap-4 px-4 pb-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] sm:items-center sm:px-5">
          ...
          <ul className="space-y-2">
            ...
            <li className="border-t border-border pt-2 text-[11px] text-muted-foreground">
              {formatCompact(total)} events total
            </li>
          </ul>
        </div>
      )}
