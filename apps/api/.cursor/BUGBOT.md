...
- **Alert rules (#493)**: `AlertRule` CRUD must scope by `project_id` + `deleted_at: null`; webhook ids in `destinations` must belong to the same project; cap with `MAX_ALERT_RULES` via atomic count+insert; evaluate enabled rules on error ingest (`maybeEvaluateAlertRules`) without removing built-in `maybeNotifyErrorSpike` / `maybeNotifyQuotaAlerts`; unknown `condition_type` values must be skipped, not crash. `ERROR_COUNT` environment scope must filter `ErrorOccurrence.environment` (populated at ingest), never the mutable last-seen `ErrorGroup.environment`.
