...
- **Alert rules (#493 / #534 / #535)**: `AlertRule` CRUD must scope by `project_id` + `deleted_at: null` + `source=CUSTOM` for list/cap; store `conditions` (AND) + opaque `destination_ids` (not provider enums); webhook uuids in `destination_ids` must belong to the same project; resolve ids via `resolveDestinationIds` → `fireProjectAlert` only (no provider send logic in rules); cap with `MAX_ALERT_RULES` via atomic count+insert on CUSTOM only; evaluate ingest conditions on error ingest (`maybeEvaluateAlertRules`) and schedule-oriented conditions via `runScheduledAlertRuleEvaluation` / `alert-rules-evaluator` job — without removing built-in `maybeNotifyErrorSpike` / `maybeNotifyQuotaAlerts`; SYSTEM built-in rows (`source=SYSTEM`, `migration_key` `builtin:*`, condition types `BUILTIN_*`) are the canonical spike/quota representation — dual-write via `saveProjectAlertSettings` / `ensureBuiltinAlertRules` on PATCH/backfill only (not on every read); `BUILTIN_*` must remain unknown to CUSTOM evaluators so mixed-deploy pods cannot fire SYSTEM rows as `ALERT_RULE`; custom evaluators must skip SYSTEM; PATCH/DELETE on SYSTEM must 403; built-in fires keep legacy `AlertEvent.rule` + dedupe keys (`alert:error_spike:…`, `quota:near|exceeded:…`) so mixed deploy cannot double-notify; concurrent ensure must tolerate P2002 on `(project_id, migration_key)`; unknown condition `type` values must be **skipped** (never crash AND evaluation); rules with no remaining supported conditions must not fire; cooldown for CUSTOM must use atomic `last_fired_at` claims (elapsed time since last fire) on both paths — not wall-clock bucket dedupe alone; scheduled sweep must skip soft-deleted projects and soft-deleted orgs (ingest parity). `ERROR_COUNT` / error-scoped environment filters must use `ErrorOccurrence.environment` (populated at ingest), never the mutable last-seen `ErrorGroup.environment`. Schedule cadence is `ALERT_RULES_SCHEDULE_INTERVAL_MINUTES` only.
