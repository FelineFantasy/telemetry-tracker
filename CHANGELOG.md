...
- **Dashboard page load** — Sessions KPI summary (`/api/sessions/summary`) no longer blocks list HTML; Overview’s performance card fetches after first paint instead of holding the document open; Releases shows a loading skeleton while the summary query runs.
