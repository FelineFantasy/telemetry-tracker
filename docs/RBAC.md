...
**Dashboard:** `/api/meta/session-context` returns `role` and boolean flags (`canResolveErrors`, `canCreateApiKey`, `canRevokeApiKey`, `canManageOrganization`, `canCreateProject`, `canManageMembers`) for the active project and sidebar organization (`X-Project-Id` / `X-Organization-Id` + session).
