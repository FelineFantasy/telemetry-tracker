...
| Dashboard billing + usage | **Banner** when Stripe status is `past_due`, `unpaid`, or `canceled` (`billingHealth` on `/api/meta/session-context`). **Near-quota banner** from `usageQuota` (≥90% monthly ingest). **Sonner toast** once per browser session with general Stripe disclaimer. |
