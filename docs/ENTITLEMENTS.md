...
| Stripe → `plan_tier` + subscription state | **Webhooks** (when `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` are set): **`checkout.session.completed`** (tier from session metadata; loads subscription for `stripe_subscription_status` + period end when possible), **`customer.subscription.updated`** (syncs status, period end, and `plan_tier` from **subscription or first price metadata** `plan_tier`), **`customer.subscription.deleted`** (FREE + clears Stripe fields), **`invoice.payment_failed`** (sets status `past_due` on the org). Org rows store `stripe_subscription_status` and `stripe_current_period_end`. **Effective limits:** `canceled`, `unpaid`, `incomplete`, and `incomplete_expired` enforce **FREE** caps; **`past_due`** keeps paid limits (Stripe dunning) but the dashboard shows a **billing banner**. |
