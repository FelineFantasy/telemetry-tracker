...
7. **Alert rules (#493)**: Custom `AlertRule` rows are project-scoped (soft-delete); destination webhook ids must belong to the project; evaluate on error ingest without removing built-in spike/quota hooks; unknown condition types must be skipped.
