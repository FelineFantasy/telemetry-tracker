...
6. **Alert webhooks**: Outbound `ProjectWebhook` URLs must be HTTPS-only; list APIs must not leak full URLs or signing secrets; delivery log list must mask URLs the same way; `fireProjectAlert` must still succeed if webhook delivery fails. Delivery must re-resolve DNS and block private/loopback targets (not hostname-string checks alone). Test webhook failures must not be logged as `DEAD`.
