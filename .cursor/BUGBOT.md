...
7. **Alert rules (#493)**: Custom `AlertRule` rows are project-scoped (soft-delete); `conditions[]` (AND) + opaque `destinationIds` (not provider enums); webhook uuids in destination ids must belong to the project; rules only call `fireProjectAlert` (Notifications owns delivery); evaluate on error ingest without removing built-in spike/quota hooks; unknown condition types must skip the rule.
