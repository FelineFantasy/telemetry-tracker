...
7. **Alert rules (#493)**: Custom `AlertRule` rows are project-scoped (soft-delete); `conditions[]` (AND) + opaque `destinationIds` (not provider enums); webhook uuids in destination ids must belong to the project; rules only call `fireProjectAlert` (Notifications owns delivery); evaluate on error ingest without removing built-in spike/quota hooks; unknown / unparsable condition types must skip the rule (never synthesize a default spike); unparsable rules exposed with empty `conditions` must still list in the Alerts UI so editors can disable/remove them; `ERROR_COUNT` environment scope must match trimmed `ErrorOccurrence.environment` from ingest.
